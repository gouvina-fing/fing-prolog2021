...
% generar_fila(+N, ?E, ?F) - Comprueba si la lista F tiene largo N y E elementos
generar_fila(1, E, [E]).
generar_fila(N, E, [E|T]) :- N > 1, N2 is N-1, generar_fila(N2, E, T).
...
matriz(1, N, E, [F]) :- generar_fila(N, E, F).
matriz(M, N, E, [F|T2]) :- M > 1, generar_fila(N, E, F), M2 is M-1, matriz(M2, N, E, T2).
